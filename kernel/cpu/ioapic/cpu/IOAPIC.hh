...
public:
    ...
    UIntField<value_t,base_t, 0,8> intvec; // interrupt vector from 0x10 to 0xFE
    ...
    uint32_t read(Register reg) {
        ...
        ioapic[0] = (reg & 0xff);
        MLOG_ERROR(mlog::boot, "IOAPIC: have read", ioapic[4], DVAR(reg));
        return ioapic[4];
    }

    void write(Register reg, uint32_t value) {
        MLOG_ERROR(mlog::boot, "IOAPIC: write", DVAR(base_address), DVAR(reg), DVAR(value));
        uint32_t volatile *ioapic = (uint32_t volatile *)base_address;
        ioapic[0] = (reg & 0xff);
        ioapic[4] = value;
    }
